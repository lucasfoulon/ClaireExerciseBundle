...
// Both or one of selector and customAttribute must be defined.
	...
	.run(['taRegisterTool', '$window', 'taTranslations', 'taSelection', function(taRegisterTool, $window, taTranslations, taSelection){
		...
		taRegisterTool('annotate', {
			buttontext: taTranslations.annotate.buttontext,
			tooltiptext: taTranslations.annotate.tooltip,
			...
			action: function(){
                ...
                for(var i = 0 ; i < lgMax ; i++) {
                    ...
                    if(preCaretRange.toString()[j] == selectParent.innerHTML[i]) {
                        j++;
                        if(preCaretRange.toString().charCodeAt(j) >= 128) {
                            //AVANT LE MOT
                            if(j<debut)
                                manque += 5;
                            //PUIS DANS LE MOT
                        ...
                        //console.log("char : "+selectParent.innerHTML.charAt(i)+" code at : "+selectParent.innerHTML.charCodeAt(i));
                        //AVANT LE MOT
                        if(j<debut+1) {
                            manque++;
